...
import ar.edu.utn.frba.ddam.homie.activities.SplashActivity
...
class Utils {
    companion object {
        ...
        fun getSplashPref(context: Context) : String {
            return PreferenceManager.getDefaultSharedPreferences(context).getString("splash", "auto")!!
        }

        fun welcomeMessage(view: View, displayName: String) {
            if(PreferenceManager.getDefaultSharedPreferences(view.context).getBoolean("welcome_message", false)) {
                Snackbar.make(
                        view,
                        "${view.context.resources.getString(R.string.welcome_message_text)} ${displayName}",
        ...
        fun getStartApp(context: Context) : Boolean {
            val sharedPreferences = context.getSharedPreferences(PREF_TAG, Context.MODE_PRIVATE)
            return sharedPreferences.getBoolean("START_APP", false)
        }

        fun setStartApp(context: Context, value: Boolean) {
            val sharedPreferences = context.getSharedPreferences(Utils.PREF_TAG, Context.MODE_PRIVATE)
            val editor = sharedPreferences.edit()
            editor.putBoolean("START_APP", value)
            editor.apply()
        }
